...
The static site output is emitted to `docs/.vitepress/dist/`. Serving the production build locally mirrors the assets that GitHub
Pages would publish.

## Formatting

The repository relies on Prettier for consistent formatting of Markdown and configuration files. Before sending changes,
normalize the files with:
...
When evaluating rendering behaviour or sanity checks that depend on seeded randomness, document the seed value in the Markdown
examples so reviewers can reproduce the same output locally. This applies to both CLI-driven workflows and GUI mockups rendered
via the VitePress site.
